...
function Footer() {
  return (
    <div className='w-full bg-zinc-900 '>
        <div className='footer-container flex p-[5vw] h-full'>
            <div className='headings flex flex-col justify-between w-1/2 '>
                <div>
            ...
            <div className='discription flex flex-col w-1/2 justify-between '>
                <h1 className='text-[6vw] font-semibold leading-none tracking-tighter pb-5'>PRESENTATIONS</h1>
                <div className='social-links'>
                    {['S:', 'Instagram', 'Behance', 'Facebook', 'LinkedIn'].map((items,index)=>  (
                        <p key= {index} className= {`text-lg ${index==1 && 'mt-6'}`}>{items}</p>
                    ))}
                </div>
                
                <div className='flex justify-between'>
                    <div className='address'>
                    {['L:', '202-1965 W 4th Ave', 'Vancouver, Canada', '30 Chukarina St', 'Lviv, Ukraine'].map((items,index)=>  (
                        <p key= {index} className= {`text-lg ${index==1 && 'mt-6'} ${index==3 && 'mt-6'}`}>{items}</p>
                    ))}
                    </div>
                    <div className='services'>
                    {['M:', 'Home', 'Services', 'Our Work', 'About Us', 'Insights' ,'Contact Us'].map((items,index)=>  (
                        <p key= {index} className= {`text-lg ${index==1 && 'mt-6'}`}>{items}</p>
                    ))}
                    </div>
                </div>
